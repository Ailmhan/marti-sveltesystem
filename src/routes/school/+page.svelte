<script lang="ts">
	...
	onMount(async () => {
		console.log('🏫 School Page onMount, authStore:', {
	...
	<div class="hero-background">
		{#if $authStore.schoolData?.logoUrl}
			<img 
				...
				alt="Фото школы" 
				class="hero-bg-image"
				on:load={() => console.log('✅ Hero background image loaded from logoUrl:', $authStore.schoolData?.logoUrl)}
				on:error={(e) => console.error('❌ Hero background image failed to load:', $authStore.schoolData?.logoUrl, e)}
			/>
		{:else}
			<div class="hero-bg-placeholder"></div>
			{#if $authStore.schoolData}
				<div style="display: none;">{console.log('ℹ️ No logoUrl found for school:', $authStore.schoolData)}</div>
			{/if}
		{/if}
